...
const server = http.createServer((req, res) => {
  ...
  res.writeHead(200, {
    ...
    'Access-Control-Allow-Origin': '*', // so client.html (a different origin) can read it
  });
...
>
> ⚠️ **This only works because `server.js` sends `Access-Control-Allow-Origin: *`.** Opening `client.html` from `file://` makes the `fetch` cross-origin; without that header the browser sends the request but blocks your JS from *reading* the response (a CORS error — see §E). That failure is itself a great thing to observe: comment the header out, reload, and watch it break.
